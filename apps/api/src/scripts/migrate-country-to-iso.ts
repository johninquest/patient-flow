 ...
 *   To rollback: pnpm run db:migrate-country -- --rollback
 */
...
async function rollback(pool: Pool): Promise<void> {
  ...
  try {
    ...
    const countResult = await pool.query(
      'SELECT COUNT(*) FROM patients_country_backup',
    );
...
async function migrate() {
  ...
  if (invalidCodes.length > 0) {
    ...
    console.log('');
    console.log(
      '   These codes are not in ISO_COUNTRY_CODES. Fix the mapping before proceeding.',
    );
    process.exit(1);
  }
  ...
  try {
    ...
    for (let i = 0; i < patients.length; i++) {
      ...
      const newAddress = isAddress(patient.address)
        ? { ...patient.address }
        : patient.address;
      const newIdentity = isIdentity(patient.identity)
        ? { ...patient.identity }
        : patient.identity;
      ...
      if (
        ...
        if (isoCode) {
          if (verbose) {
            console.log(
              `  Patient ${patient.id}: address.country "${newAddress.country}" → "${isoCode}"`,
            );
          }
          ...
          if (verbose) {
            console.log(
              `  ⚠️  Patient ${patient.id}: address.country "${newAddress.country}" not found`,
            );
          }
        }
      ...
      if (
        isIdentity(newIdentity) &&
        newIdentity.country_national &&
        !isIsoCode(newIdentity.country_national)
      ) {
        ...
        if (isoCode) {
          if (verbose) {
            console.log(
              `  Patient ${patient.id}: identity.country_national "${newIdentity.country_national}" → "${isoCode}"`,
            );
          }
          ...
          if (verbose) {
            console.log(
              `  ⚠️  Patient ${patient.id}: identity.country_national "${newIdentity.country_national}" not found`,
            );
          }
        }
    ...
    if (!dryRun && updated > 0) {
      ...
      console.log('  pnpm run db:migrate-country -- --rollback');
    } else if (dryRun) {
      console.log('');
